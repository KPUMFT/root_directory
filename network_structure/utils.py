...
def conv(input_tensor , name , kernel_width ,
  ...
  conv_layer = tf.nn.conv2d(input_tensor ,
    weights , (1, stride_horizontal ,
    stride_vertical , 1), padding=’SAME ’)
  activation = activation_fn(tf.nn.bias_add(
    conv_layer , biases), name=name)
  return activation


def fully_connected(input_tensor , name , output_neurons
  , activation_fn=tf.nn.relu):
  n_in = input_tensor.get_shape()[ -1].value
  with tf.variable_scope(name):
    weights = tf.get_variable(’weights ’, [n_in ,
    ...
    biases = tf.get_variable("bias", [
      ...
      constant_initializer (0.0))
    logits = tf.nn.bias_add(tf.matmul(input_tensor
      , weights), biases , name=name)
    if activation_fn is None:
      return logits
    return activation_fn(logits)

def max_pool(input_tensor , name , kernel_height ,
  kernel_width , stride_horizontal , stride_vertical):
    return tf.nn.max_pool(input_tensor ,ksize=[1, kernel_height ,kernel_width , 1],strides=[1,stride_horizontal ,stride_vertical , 1],padding=’VALID ’,name=name)
