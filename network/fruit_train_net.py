...
def train_model(session , train_operation ,
    ...
    for i in range(1, iterations + 1):
        ...
        batch_x = np.reshape(batch_x , [network.
...
def calculate_intermediate_accuracy_and_loss(session ,
    ...
    while True:
        try:
            test_batch_x , test_batch_y = session.run(
                test_images_with_labels)
            test_batch_x = np.reshape(test_batch_x ,
...
if __name__ == ’__main__ ’:
    ...
    with tf.Session().as_default() as sess:
        ...
        test_iterator = tf.data.Iterator.
            from_structure(test_dataset.output_types ,
            test_dataset.output_shapes)
        test_init_op = test_iterator.make_initializer(
            test_dataset)
        iterator_map = {" train_iterator ":
            train_iterator , " test_iterator ":
            test_iterator , " test_init_op ": test_init_op
            }

        train_op , loss , correct_prediction = network.
            build_model()
        ...
        if useCkpt:
            ...
            train_model(sess , train_op , loss ,
