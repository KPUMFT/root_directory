...
def setup_test_data():
    ...
    with open(constants.root_dir + ’\\ utils \\ labels ’)
        as f:
...
def test_model(sess , pred , iterator , total_images ,
    file_name):
    ...
    while True:
        try:
            batch_x , batch_y = sess.run(
                images_with_labels)
            batch_x = np.reshape(batch_x , [-1, network
                .input_size])
            
            results = sess.run(pred , feed_dict={
            ...
            for i in range(len(results)):
                if not results[i]:
                    mislabeled[labels_text[batch_y[i
                        ]]] += 1

            correct = correct + numpy.sum(results)
            print(" Predicted %d out of %d; partial
                accuracy %.4f" % (correct ,
...
if __name__ == ’__main__ ’:
    ...
    with tf.Session() as sess:
        ...
        sess.run(init)
        tfrecords_files = [(constants.data_dir + f)
            for f in os.listdir(constants.data_dir) if
            re.match(file_name , f)]
        dataset = build_dataset(tfrecords_files ,
            network.batch_size)
        iterator = dataset.make_one_shot_iterator()
        ckpt = tf.train.get_checkpoint_state(constants
            .fruit_models_dir)
        saver.restore(sess , ckpt.model_checkpoint_path
            )
        test_model(sess , prediction , iterator ,
